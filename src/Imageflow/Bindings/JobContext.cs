...
public sealed class JobContext : CriticalFinalizerObject, IDisposable, IAssertReady
{
    ...
    public ImageInfo GetImageInfo(int ioId)
    {
        ...
        if (node == null)
        {
            throw new ImageflowAssertionFailed("get_image_info response is null");
        }

        var responseObj = node.AsObject();
        if (responseObj == null)
        {
            throw new ImageflowAssertionFailed("get_image_info response is not an object");
        }
    ...
    public VersionInfo GetVersionInfo()
    {
        ...
        if (node == null)
        {
            throw new ImageflowAssertionFailed("get_version_info response is null");
        }
        ...
        ReadOnlySpan<byte> utf8Json, CancellationToken cancellationToken = default)
    {
        ...
        if (nullTerminatedMethod.Length == 0)
        {
            throw new ArgumentException("Method cannot be empty", nameof(nullTerminatedMethod));
        }

        if (nullTerminatedMethod[^1] != 0)
        {
    ...
    public void AddInputBytes(int ioId, ArraySegment<byte> buffer)
    {
        ...
        {
            throw new ArgumentNullException(nameof(buffer), "Array cannot be null");
        }
    ...
    {
        AssertReady();
        if (ContainsIoId(ioId))
        {
            throw new ArgumentException($"ioId {ioId} already in use", nameof(ioId));
        }

...
public class BuildJobResult
{
    ...
    internal static BuildJobResult From(IJsonResponse response, Dictionary<int, IOutputDestination> outputs)
    {
        ...
        if (data == null)
        {
            throw new ImageflowAssertionFailed("BuildJobResult.From cannot parse response ('data' missing) " + response.CopyString());
        }
        ...
        if (jobResult == null)
        {
            throw new ImageflowAssertionFailed("BuildJobResult.From cannot parse response (missing job_result or build_result) " + response.CopyString());
        }
